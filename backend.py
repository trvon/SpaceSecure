...
#I'll be back first thing tommorow morning to clean up code and help with anything we have then
#changeattempt is setup to make auth change attempts with code, and secure test to see if change is possible.
#cannot handle two password change requests at once currently, but UI also cannot send them.
#GUI should no longer do any more import scanning, and will recieve the list of entries as an array value 
# following code should be work(didn't want to push because I don't think GUI is latest)
#
# inputList = backend.getDeviceList                
# for entry in inputList:
#	self.tree.insert("",0, value = (entry[0], entry[1], entry[2], entry[3])
# 
#/may need to be adjusted if code has been changed



#defines and returns list of login/password combinations derived from the MIRAI exploit(https://github.com/jgamblin/Mirai-Source-Code)
...
#alternate auth setup that takes lines from a file
...
#must be run once for other functions in the program to work(could fix later with try:catch), takes opitional runtime ARG for the location of the sql server, or uses a default location
def initialSetup():
	# commonAuthSetup()
...
#called by frontend to write a txt file of scanned devices on the network and their security status
def getDeviceList():
    #calls the SH script that scans the network
	os.system("./discoverDevices.sh")
...
#this should attempt to connect to target ip with the provided username and password combination, authcode[0] and authcode[1], and then change the password to newpass, returning false if the attempt fails
def secureTest(target):
	if os.system("./sshVulnerabilityCheck.sh " + target) == 1:
		return True	
	else:
		return False


#takes a list of IP's determined to be insecure and new passwords to replace them with.
